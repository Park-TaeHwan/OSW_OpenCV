...
while True:
    ...
    if time.time()-timeStart < totalTime:
        ...
        if hands:
            ...
            if distanceCM < 40:
                if x < cx < x + w and y < cy < y + h:
                    counter = 1
            cv2.ellipse(img, (x/2,y/2), (x,y), rad(coordinate), rad(coordinate), rad(coordinate)/2, color)
        ...
        # Draw Button
        cv2.ellipse(img, (x/2,y/2), (x,y), rad(coordinate), rad(coordinate), rad(coordinate)/2, color, cv2.FILLED)  
